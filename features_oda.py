...
## Forecast Features
def add_hour_forecast_features(data, labels=[], lags=[]):
  for label in labels:
    for lag in lags:
...
def add_features(data, labels, weather_labels):
  ...
  add_hour_forecast_features(data, labels=weather_labels, lags=[-1, -2, -3, -4, -5, -6, -8, -9, -10, -11, -12])
