...
def preprocess(**config):
  ...
  the_rest = ['Bakke kirke','Torvet', 'E6-Tiller', 'Elgeseter'] # do nothing
  test_size = config.get('test_size', 0.3)
  val_size = config.get('val_size', 0.1)
  shuffle = config.get('shuffle', True)
  window = config.get('window', 6)

  if (1 == 2 ): #os.path.exists(cache_path)):
    ...
    df1 = df[[*stations, 'weather', 'Bakke kirke', 'Elgeseter', 'E6-Tiller']]
    #df2 = df[[]]
    ...
    df = add_features(df, ['Torvet PM10', 'Torvet PM2.5' ]) # 'Torvet PM10']
...
def get_targets(df, pred_var, window):
  ...
  for x in range(1, window + 1):
    ...
    temp[new_label] = df["Torvet PM10"].shift(-x)
